...
description: An honest comparison of git-scenarios against hand-rolled fixtures, checked-in .git directories, mocking git, hand-rolled mock objects, Docker containers, and the library's own mock factories.
---
...
const fakeStatus = {
  ...
  modified: [],
  not_added: [],
  files: [{ path: 'src/auth.ts', index: 'M', working_dir: ' ' }],
  current: 'main',
  isClean: () => false,
...
// Wire into a full SimpleGit mock
const git = mockSimpleGit({ createMockFn: jest.fn, overrides: { status } })
```

Every returned object has all required fields populated with correct defaults. The bucket-to-XY-code mapping is handled internally, so `staged('x.ts')` produces a `FileStatusResult` with `index: 'M', working_dir: ' '` — matching exactly what `simple-git` would return. No type casting, no missing fields, no drift.
...
<Aside type="tip">
**Rule of thumb:** If the test exercises git *behavior* (does a merge produce a conflict?), use a real repo. If the test exercises your code's *reaction* to git state (does the UI render a conflict badge?), use a mock factory. If you only need a status object for a quick type check in a utility function, hand-rolled is fine — but consider whether a one-liner mock builder is just as easy.
</Aside>
...
- [Mock Factories →](/docs/guides/mock-factories) — in-memory mocks for unit tests without disk I/O
