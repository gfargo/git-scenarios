...
<Aside type="tip">
`repoFixture` is a thin factory — the consumer owns the `test` import, so the package has no hard dependency on `vitest`. Compose it with your own fixtures using `base.extend({ ...repoFixture('name'), ...yourOwnFixtures })`.
</Aside>
...
<Aside>
Captured a real repo with [`git-scenarios capture`](/docs/guides/cli#git-scenarios-capture-path-options)? The emitted module is a `defineScenario(...)` call — register it the same way.
</Aside>
